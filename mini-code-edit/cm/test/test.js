...
testCM("selection", function(cm) {
    ...
    eqPos(cm.getCursor(false), {
        line: 2,
        ch: 2
    });
    eqPos(cm.getCursor(true), {
        line: 0,
        ch: 4
    });
    cm.setSelection({
...
testCM("markTextSingleLine", function(cm) {
    forEach([{
        ...
        var f = r.find();
        eq(f.from && f.from.ch, test.f);
        eq(f.to && f.to.ch, test.t);
    });
});
...
testCM("bookmark", function(cm) {
    ...
    forEach([{
        ...
        cm.setValue("1234567890\n1234567890\n1234567890");
        var b = cm.setBookmark(p(test.bm) || {
            line: 1,
            ch: 5
        });
        cm.replaceRange(test.c, p(test.a), p(test.b));
