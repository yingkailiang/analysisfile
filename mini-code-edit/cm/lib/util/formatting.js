    ...
    autoFormatLineBreaks: function(text) {
        ...
        if (nonBreakableBlocks != null) {
            ...
            for (var i = 0; i < nonBreakableBlocks.length; i++) {
                ...
                if (nonBreakableBlocks[i].start <= curPos && nonBreakableBlocks[i].end >= curPos) { // Skip non-breakable block
        ...
        if (modeInfos.length > 1) { // Deal with multi-mode text
            for (var i = 1; i <= modeInfos.length; i++) {
                ...
                if (modeInfos[i - 1].modeName != "xml") { // Starting a CSS or JavaScript block
                    if (!reBlockStartsWithNewline.test(textPortion) && selStart > 0) { // The block does not start with a line break
                        textPortion = "\n" + textPortion;
                    }
                    if (!reBlockEndsWithNewline.test(textPortion) && selEnd < text.length - 1) { // The block does not end with a line break
                        textPortion += "\n";
                    }
                }
