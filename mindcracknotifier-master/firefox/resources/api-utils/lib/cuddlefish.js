...
function Loader(options) {
  ...
  options = override(options, {
    ...
    resolve: function resolve(id, requirer) {
      let entry = requirer && requirer.id in manifest && manifest[requirer.id];
      let uri = null;
      ...
      if (entry) {
        ...
        if (!requirement)
        {
        	Components.utils.reportError('Module: ' + requirer.id + ' located at ' + requirer.uri                      + ' has no authority to load: ' + id, requirer.uri);
         // throw Error('Module: ' + requirer.id+ ' located at ' + requirer.uri  + ' has no authority to load: ' + id, requirer.uri);
		return id;
		}
        uri = requirement;
      }
