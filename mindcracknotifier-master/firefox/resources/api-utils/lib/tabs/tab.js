...
const TabTrait = Trait.compose(EventEmitter, {
  ...
  _changeLocation: defer(function(url) {console.warn("@fixme: TabTrait._changeLocation",url);this._browser && this._browser.loadURI(url)}),
  /**
  ...
  for (let i = 0; i < topWindow.gBrowser.browsers.length; i++) {
    ...
    if (getWindowID(w) == topWindowId) {
      return Tab({
        ...
        window: require("addon-kit-sf/windows").BrowserWindow({ window: topWindow }),
