...
exports.Worker = Worker.resolve({
	constructor : '__initWorker'
}).compose({
	constructor : function Worker(options) {
		try {
			var window = options.window;
			if ("_frame" in this || options.singleUse || this._singleUse) {
				return this.__initWorker(options);
			}
			var windowID = window.QueryInterface(Ci.nsIInterfaceRequestor).getInterface(Ci.nsIDOMWindowUtils).currentInnerWindowID;
			if ( windowID in Workers) {
				var worker = Workers[windowID];
				worker._contentWorker.process(options);
				return worker;
			} else {
				if ('contentScriptContext' in options)
					this.contentScriptContext = options.contentScriptContext;
				this.__initWorker(options);
				Workers[windowID] = this;
				this.on("detach", function() {
					delete (Workers[windowID]);
				});
				return this;
			}
		} catch(err) {
