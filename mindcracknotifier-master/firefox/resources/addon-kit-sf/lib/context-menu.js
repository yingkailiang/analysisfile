...
function SelectorContext(selector,targetPatterns) {
  let opts = apiUtils.validateOptions({ selector: selector,targetPatterns:targetPatterns }, {
    selector: {
      is: ["string"],
      msg: "selector must be a string."
    },
    targetPatterns:{
      map: function (v) apiUtils.getTypeOf(v) === "array" ? v : (v==undefined ? []:[v]),
      ok: function (v) v.every(function (p) (typeof(p) === "string")|| (p && (typeof(p.test)==="function"))),
      msg: "patterns must be a string or an array of strings/regexp."
    }
  });
  try {
    targetPatterns = opts.targetPatterns.map(function (p) new MatchPattern(p));
  }
  catch (err) {
    console.error("Error creating SelectorContext match pattern:");
    throw err;
  }
  ...
  this.isCurrent = function SelectorContext_isCurrent(popupNode) {
  	var node=closestMatchingAncestor(popupNode);
  	return !!node && (node.src?this.isCurrentForURL(node.src):(node.href?this.isCurrentForURL(node.href):true)); 
  };
  this.isCurrentForURL = function URLContext_isCurrentForURL(url) {
    return targetPatterns && targetPatterns.length>0 ? targetPatterns.some(function (p) p.test(url)):true;
  };
...
function SomeContext(contexts) {
  this.isCurrent = function SomeContext_isCurrent(popupNode) {
  	return contexts.some(function(context){return context.isCurrent(popupNode)})
  };
}

SomeContext.prototype = new Context();

exports.SomeContext = apiUtils.publicConstructor(SomeContext);
...
WorkerRegistry.prototype = {
  ...
  _makeWorker: function WR__makeWorker(win) {
    ...
    worker.on("message", function workerOnMessage(msg) {
      try {
        privateItem(item)._emitOnObject(item, "message", msg,worker);
      }
      catch (err) {
        console.exception(err);
      }
