...
const PageMod = Loader.compose(EventEmitter, {
  ...
  constructor: function PageMod(options) {
    ...
    if ('allFrames' in options)
      this.allFrames = options.allFrames;      
    ...
    if ('onContent' in options)
        this.on('content', options.onContent);
  ...
  _onContent: function _onContent(window) {
  	if(!this.allFrames && window.top!=window){
 // we do not want to inject content script in inner frames
  		return;
  	}
    // not registered yet
  ...
  _createWorker: function _createWorker(window) {
    let worker = Worker({
      ...
      contentScriptContext: this.contentScriptContext,
      contentScript: this.contentScript,
      contentScriptFile: this.contentScriptFile,
      contentScriptOptions: this.contentScriptOptions,
      onError: this._onUncaughtError
    });
    this._emit('attach', worker,window.top==window);
    let self = this;
