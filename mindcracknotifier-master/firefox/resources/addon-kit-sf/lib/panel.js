...
const Panel = Symbiont.resolve({
  ...
  set _frameLoadersSwapped(value) {
    ...
    if (!this._frame) {
    	console.error("_frameLoadersSwapped : no iframe !")
    	return;}
  ...
  show: function show(anchor) {
    ...
    // we need additional space on right
    xulPanel.firstChild.style.marginRight="7px";
  ...
  resize: function resize(width, height) {
    ...
    this.height = height;
    if(this._xulPanel){
    	//this._xulPanel.firstChild.style.MozTransition ="all 0.1s ease-in-out";
    	this._xulPanel.firstChild.style.width = this.width + "px";   
	    this._xulPanel.firstChild.style.height = this.height + "px";
	    /*var domWindow=this._xulPanel.firstChild.contentWindow;
	    if(domWindow){
	    	BrowserWindow({window:domWindow,type:"panel"});
	    }
	    else{
	    	console.error("resize : no domWindow for panel ", this._xulPanel)
	    }
	    */
 	 }
  ...
  __xulPanel: null,
  get _viewFrame() this.__xulPanel && this.__xulPanel.children[0], 
  /**
