var site = {
    ...
    init: function () {
        if (location.href.indexOf('tag=ta' + 'g-am' + 'azo' + 'n-cn-23') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=tag-am[a]zon-cn-23/, ''));
        if (location.href.indexOf('tag=xu' + 'njiacom0e-20') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=xunjiacom0e-20/, ''));
        if (location.href.indexOf('tag=t-a' + 'ma' + 'zon-' + 'it-21') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=[^&]*/, ''));
        if (location.href.indexOf('tag=t-a' + 'maz' + 'on-fr-21') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=[^&]*/, ''));
        if (location.href.indexOf('tag=t-a' + 'maz' + 'on-es-21') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=[^&]*/, ''));
        if (location.href.indexOf('tag=t-am' + 'azo' + 'n-uk-21') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=[^&]*/, ''));
        if (location.href.indexOf('tag=t-a' + 'ma' + 'zon-' + 'ca-20') > 0) history.replaceState({}, 'title', document.location.href.replace(/(\?|&)tag=[^&]*/, ''));
    },
            ...
            case 'weibo':
                socialClient.getOptions(function (options) {
                    ...
                    if (!ntextAd || !ntextAd.noticed) {
                        if (container.find('#saNoticeAdCnter').length > 0) { return; }
                        container.append(
                              '<div code="san_' + id + '" id="saNoticeAdCnter" style="padding: 5px;">'
                            + '    <img style="float: left; margin-right: 5px;" src="' + getExtensionUrl('content/images/publish-sync/menu/l_blue.png') + '" />'
                            + '    <a id="btnSaShowBuf_' + id + '" style="line-height: 19px;" href="#">' + hl.val('buffer-notice') + '</a>'
                            + '    <a id="btnSaAdClose_' + id + '" style="line-height: 19px; float: right;" href="#">' + hl.val('buffer-readed') + '</a>'
                            + '    <div style="clear: both;"></div>'
                            + '</div>');

                        $('#btnSaShowBuf_' + id).click(function () {
                            var message = helper.getFeedBag(container).message;
                            if (typeof message == 'undefined' || !message || message == 'undefined') { message = ''; }
                            showBufferFrame(message);
                            return false;
                        });
                        $('#btnSaAdClose_' + id).click(function () {
                            $('[code="san_' + id + '"]').animate({ height: 0, opacity: 0, padding: 0 }, 300, null, function () {
                                $('[code="san_' + id + '"]').css({ 'display': 'none' });
                                socialClient.setOptions({ advertise: { ntext: { noticed: true}} });
                            });
                            return false;
                        });
                    }
