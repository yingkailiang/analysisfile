 ...
 * @author Jack.wu.xu
 */
public class MoveRestoreResource extends CommonAction {
		...
		try {
			for (String id : dids) {
				if (!StringUtils.isEmpty(id)) {
					String deleteDir = sbox.movRestoreDir(id, targetId, secretKey);
			...
			for (String id : fids) {
				if (!StringUtils.isEmpty(id)) {
					String moveObject = sbox.movRestoreObject(id, targetId, secretKey);
					net.sf.json.JSONObject fromObject = net.sf.json.JSONObject
