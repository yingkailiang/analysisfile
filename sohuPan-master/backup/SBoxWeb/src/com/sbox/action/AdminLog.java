...
public class AdminLog extends CommonAction {
	...
	public String toPage() {
		try {
			...
			String getlog = sc.getAdminUserLog(sboxAppLog.getStartTime(), sboxAppLog.getEndTime(),userEmail,sboxAppLog.getStatus(), String.valueOf(sboxAppLog.getResourceType()), String.valueOf(sboxAppLog.getFlag()), sboxAppLog.getPageIndex(), sboxAppLog.getPagenum(), secretKey);
			JSONObject js = JSONObject.fromObject(getlog);
			logs = js.getJSONArray("sboxBusLogList");
			page.setAllMessage((Integer) js.get("total"));
			putSession(SessionName.ADMINLOGSESSION, page);
	...
	public String nextPage() {
		try {
			SecretKey secretKey = getSecretKey();
			SBoxClient sc = SBoxClientInstance.getSboxClient();
			page = (Page) getSession(SessionName.ADMINLOGSESSION);
			page.nextPage();
